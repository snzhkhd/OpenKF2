...
void sub_8005BCF4(uint8_t* rdram, recomp_context* ctx) {
...
L_8005BD34:
    ...
    // lhu         $v0, 0x10($v1)
    ctx->r2 = MEM_HU(0X10, ctx->r3);
    // nop

    // sh          $v0, 0x10($s0)
    MEM_H(0X10, ctx->r16) = ctx->r2;
    // lhu         $v0, 0x12($v1)
    ctx->r2 = MEM_HU(0X12, ctx->r3);
    // nop

    // sh          $v0, 0x12($s0)
    MEM_H(0X12, ctx->r16) = ctx->r2;
    // lhu         $v0, 0x14($v1)
    ctx->r2 = MEM_HU(0X14, ctx->r3);
    // nop

    // sh          $v0, 0x14($s0)
    MEM_H(0X14, ctx->r16) = ctx->r2;
    // lhu         $v1, 0x16($v1)
    ctx->r3 = MEM_HU(0X16, ctx->r3);
    // addu        $v0, $zero, $zero
    ctx->r2 = ADD32(0, 0);
    // sh          $v1, 0x16($s0)
