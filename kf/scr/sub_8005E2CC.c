...
void sub_8005E2CC(uint8_t* rdram, recomp_context* ctx) {
    ...
    // lwl         $t2, 0x7($a0)
    ctx->r10 = do_lwl(rdram, ctx->r10, 0X7, ctx->r4);
    // lwr         $t2, 0x4($a0)
    ctx->r10 = do_lwr(rdram, ctx->r10, 0X4, ctx->r4);
    // swl         $v1, 0x3($v0)
    do_swl(rdram, 0X3, ctx->r2, ctx->r3);
    // swr         $v1, 0x0($v0)
    do_swr(rdram, 0X0, ctx->r2, ctx->r3);
    // swl         $t2, 0x7($v0)
    do_swl(rdram, 0X7, ctx->r2, ctx->r10);
    // swr         $t2, 0x4($v0)
    do_swr(rdram, 0X4, ctx->r2, ctx->r10);
    // lui         $v0, 0x8007
    ctx->r2 = S32(0X8007 << 16);
    // lw          $v0, 0x63AC($v0)
    ctx->r2 = MEM_W(0X63AC, ctx->r2);
